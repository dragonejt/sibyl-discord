...
export default function embedMessageModeration(
    message: Message,
    action: number,
    reasons: Reason[]
) {
    ...
    if (ACTIONS[action] === "BAN")
        embed.setDescription(`Banned <@${message.author.id}>`);
    else if (ACTIONS[action] === "KICK")
        embed.setDescription(`Kicked <@${message.author.id}>`);
    else if (ACTIONS[action] === "MUTE")
        embed.setDescription(`Muted <@${message.author.id}>`);
    else if (ACTIONS[action] === "REMOVE")
        embed.setDescription("Removed Message");
