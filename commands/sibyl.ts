...
const data = new SlashCommandBuilder()
  ...
  .addChannelOption((option) =>
    option.setName("log_channel").setDescription("Channel to Send Logs"),
  )
  .addRoleOption((option) =>
    option.setName("notify_role").setDescription("Role to Notify"),
  );

async function execute(interaction: ChatInputCommandInteraction) {
  await interaction.reply(
    `Sibyl Pong! Response Time: ${Date.now() - interaction.createdTimestamp}ms`,
  );
  if (interaction.memberPermissions?.has(PermissionFlagsBits.Administrator)) {
    ...
    if (interaction.options.get("notify_role")) {
      ...
      interaction.followUp(
        `Notification Target has been updated to <@&${(await Communities.read(interaction.guildId!))?.discord_notify_target}>`,
      );
    }
  } else if (
    interaction.options.get("log_channel") ||
    interaction.options.get("notify_role")
  ) {
    interaction.followUp({
      content:
